...
* **F-C3 (Medium)** — job **override** accepts `_roleHasTab('HOUSEKEEPING', 2)` while force-vacant is `HK_MANAGER_ROLES` only → a housekeeper refused from freeing a room can override the job that holds it. Owner decision needed, then align both paths.
