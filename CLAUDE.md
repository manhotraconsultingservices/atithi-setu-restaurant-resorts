...
- **Backend (the security boundary — the FE is only cosmetic):** gate every mutating endpoint with `requireTabAction(TAB, ACTION)` — `'CREATE'` for POST, `'UPDATE'` for PUT/PATCH, `'DELETE'` for DELETE. **Two ways this silently breaks, both found live (Sept 2026 — a View grant could write ~111 endpoints via API while the UI hid the buttons):**
  ...
  - `requireTabAction` is **inert on routes with no `:id` param** (`if(!tenantId) return next()`) — e.g. `/api/owner/*`, `/api/orders/:id` (`:id` is the order, not the tenant). For those, add an inline check inside the handler: `_roleHasTab(req, TAB, minLevel)` (resolves tenant from `req.user.restaurantId`). But `_roleHasTab` **denies a MANAGER whose matrix is unconfigured**, so on owner-scoped routes pair it with an allowlist: `if (!['OWNER','MANAGER','SUPER_ADMIN','CTO'].includes(role) && !(await _roleHasTab(req, TAB, 2))) return res.status(403)…`. Keep `role-permissions` / `custom-roles` / module-`enable` **owner-only** — never tab-gated — or a role could self-escalate.
