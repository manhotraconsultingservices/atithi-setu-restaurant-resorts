...
* **The 11-to-11 rule is a CHARGE, not a block:** `hotel_late_checkout_time` = `11:00` auto-adds one extra night as a visible folio line past the cutoff on the departure date. **There is no check-in TIME setting** — check-in is blocked only before the check-in DATE, there is no early-arrival charge, and no arrival time is published to guests.
