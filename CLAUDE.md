...
- **Backend (the security boundary — the FE is only cosmetic):** gate every mutating endpoint with `requireTabAction(TAB, ACTION)` — `'CREATE'` for POST, `'UPDATE'` for PUT/PATCH, `'DELETE'` for DELETE. **Two ways this silently breaks, both found live (Sept 2026 — a View grant could write ~111 endpoints via API while the UI hid the buttons):**
  - `requireTabAccess(TAB)` **is** `requireTabAction(TAB,'READ')` — READ-level, so a View(1) grant PASSES it. **Never guard a write with `requireTabAccess`.** (A read-ish POST that renders/searches/previews and mutates nothing may stay `requireTabAccess`.)
  - Bare `authenticate` (no tab gate) on a write = *any logged-in user can write*. Every mutating route needs a tab gate on top of `authenticate`.
  - `requireTabAction` is **inert on routes with no `:id` param** (`if(!tenantId) return next()`) — e.g. `/api/owner/*`, `/api/orders/:id` (`:id` is the order, not the tenant). Worse: when a route's `:id` is a **record id, not the tenant** (`/api/inventory/ingredients/:id`, `/api/brand/:id`), `requireTabAction('TAB',…)` looks up perms for tenant=`<record-id>` → **fail-open**. For BOTH cases use the canonical inline write-gate **`_requireTabWrite(req, res, TAB, minLevel)`** (server.ts, next to `_roleHasTab`): OWNER/MANAGER/SA/CTO fast-path + `_roleHasTab(req,TAB,minLevel)` + a 403 — `minLevel` = 2 (create/update) or 3 (delete); it resolves the tenant from `req.user.restaurantId`. Usage: `if (!(await _requireTabWrite(req, res, 'INVENTORY', 3))) return;`. (Raw `_roleHasTab` alone denies a null-matrix MANAGER — `_requireTabWrite` restores the fast-path.) Keep `role-permissions` / `custom-roles` / module-`enable` **owner-only** — never tab-gated — or a role could self-escalate.
  - A **module gate is not a write gate.** `spaStaff` / `hotelStaff` / `eventsStaff` / `restaurantStaff` / `requireModuleAccess` only prove the role can reach the *module*, not the per-tab action level — a View-in-that-module role passes. So does a **bespoke inline helper that admits ≥View or "any staff"** (the accounting `_acctOwnerOnly` admitted ≥View on any finance tab, `_acctStaff` admitted any staff — both wrongly guarded WRITES). A write needs an explicit per-tab **Edit(2)/Full(3)** check (`requireTabAction`, `_roleHasTab≥2`, or `_requireTabWrite`) on top of any module gate.
  - A **scoped custom role** (`custom_roles.scope` = HOTEL/RESTAURANT/SPA/EVENTS) only shows in-scope tabs in the Staff-Access grid (out-of-scope cells render as immutable N/A), but stale **out-of-scope grants** in its stored `tab_permissions` are STRIPPED at read time in `getTabPermissionsForRole` (mirrors the grid's `tabAllowedForRole`). Enforcement therefore matches the grid — never rely on an out-of-scope stored grant, and keep the `_HOTEL_ONLY_TABS`/`_RESTAURANT_ONLY_TABS` sets in lockstep with App.tsx `PERMISSIBLE_TABS` `*Only` flags.
- **Frontend (so a View user is never led into a 403):** import `canWriteTab(TAB)` / `canDeleteTab(TAB)` from `src/perm.ts` (they read the `tab_perms` map App.tsx mirrors to localStorage; OWNER/SUPER_ADMIN/CTO fast-path to Full). Gate create/update controls on `canWriteTab(TAB)` and delete controls on `canDeleteTab(TAB)` — hide the button or `disabled={!canWriteTab(TAB)}` the input. Detached/module components that don't have App's `canDo` in scope MUST use `perm.ts` (EventViews/SpaViews use their local `evCanEdit`/`canWriteTab`). Add a guard (`if (!canWriteTab(TAB)) return;`) at the top of the save handler too — **never** optimistically mutate local state *before* the server call, or a rejected write lingers in the UI looking applied (the `saveLang` bug).

**Landmines:** (1) use the **exact** tab id string — a wrong id makes `tab_perms[wrongId]` undefined → the control hides for MANAGER too (MANAGER is NOT in the OWNER fast-path; it passes only via matrix level≥min OR a null/unconfigured matrix). (2) `requireTabAction` fast-paths OWNER/SUPER_ADMIN/CTO only; MANAGER must resolve through the matrix.

**Verify — don't trust the buttons.** The module-isolation audit (`node test-scripts/access_audit.mjs --tenant=… --full`) proves *module* isolation but NOT action-level. To prove a View grant can't write, run an **action-level probe** (see `qa_action_probe.mjs`): mint a View(level-1) + Edit(level-2) throwaway role, POST empty bodies to each write endpoint, assert **View→403 and Edit≠403** (Edit≠403 guards against over-gating). Definition of done for a new page: a View role sees zero write controls **and** the probe shows 403; an Edit role sees create/update (not delete); a Full role sees all. To catch the WHOLE class at once instead of one bug report at a time, run a **comprehensive one-pass sweep**: static-inventory every `app.post/put/patch/delete` route + its gate → probe every non-good suspect with a **View-EVERYTHING** role (all tabs granted at level 1) → review the survivors (self-service/`/me/*`/read-only-preview/webhook are legitimate false positives). The Sept-2026 sweep found + closed **27** holes this way (a HIGH `isAdmin`-missing admin route + the whole Inventory family + brand/alert-rules/attendance/checklist writes); scripts live in the session scratchpad (`rbac_inventory.mjs` / `rbac_probe_all.mjs`).

**Granular capabilities — the "add without edit/remove" (append-only) pattern.** A single tab's Edit level grants create + update **together**, so you *cannot* express "may ADD to a record but may NOT edit or remove its existing lines" using that record's own tab. When a workflow needs that (e.g. floor staff appending guest-requested extras to a booking they otherwise can't touch), give the capability its **own machinery**: a SEPARATE permission tab, a SEPARATE append-only table (so the record's own edit/PUT can never clobber it), and DEDICATED endpoints (append gated `CREATE`/Edit, soft-void gated `DELETE`/Full — never a hard delete). Do NOT overload the record's edit permission. Reference impl = **Event Add-ons / Supplements**: tab `EVENTS_ADDONS`, table `event_booking_addons`, `POST`/`DELETE /events/bookings/:bid/addons` + a live-window status guard (CONFIRMED/IN_PROGRESS); grant `EVENTS_ADDONS`=Edit + `EVENTS_BOOKINGS`=View for add-only staff. Its lines flow into the same bill/invoice via a defensive `.catch(() => [])` query so a missing table can never break billing.

**Event booking required fields.** A STAFF-created event booking (`POST /events/bookings`) MUST have `venue_id` + `customer_phone` — enforced on BOTH the backend (400) and the New-Booking form; don't regress. The public-page inquiry (`/events/inquiry`, a lead) is the deliberate exception: phone required, venue optional (the property assigns a venue when quoting).
...
### Data-driven filter options — derive from the rows, don't hardcode the enum
Filter chips / dropdowns for **open-ended dimensions** — booking source/channel, room category, department, agent, tender type, tag, therapist, item category — MUST be derived from the distinct values actually present in the loaded rows, never a hardcoded `<option>`/chip list:
```
[...new Set(rows.map(r => r.field || 'DEFAULT').filter(Boolean))].sort()
```
An option then appears **only** when ≥1 loaded row carries it, and a brand-new value (a first-ever Agoda booking, a new OTA channel, a freshly-created department) shows up automatically with zero code change — a hardcoded list silently omits it. Keep the derivation **consistent with the filter predicate**: same normalisation/defaulting on both sides (e.g. the Groups source filter derives `g.booking_source || 'DIRECT'` because its predicate compares `(g.booking_source||'DIRECT') !== selected`). A pretty label is a lookup on the derived value (`LABELS[s] || s`), not a reason to hardcode the set. **Exception — fixed lifecycle/state enums stay hardcoded:** booking status (`BOOKED/CHECKED_IN/…`), invoice state (`UNPAID/PAID/PRINTED`), PO state (`DRAFT/SENT/RECEIVED`) are the domain's finite, known states and you *want* to offer a status that currently has zero rows so staff can filter to it. Rule of thumb: if a new legitimate value could arrive from data you don't control (an OTA, a user-created record), derive it; if the set is a closed state machine you defined in code, hardcode it. Most list filters already follow this (`[...new Set(...)]` over `uniqueDepts`, item categories, reservations Source); the Groups-source `<select>` was the last hardcoded holdout (fixed).
...
### Aiosell channel-manager integration — how ARI/reservations flow, and the rules
Atithi-Setu (PMS) ↔ **Aiosell** (channel manager → OTAs). The PMS is the **source of truth**. UI lives in **Channel Manager & RMS** sub-tabs: **🔌 Aiosell** (`AiosellPanel`) and **📜 Sync Log** (`AiosellSyncLog`). Client code: `aiosellClient.ts`; server block: search `AIOSELL CHANNEL MANAGER`. Non-obvious rules that keep breaking if forgotten:
- **What flows which way.** PMS → Aiosell pushes **availability + rates only** — an *absolute, idempotent* upsert of the room-**type** count. It does **not** send guest names/reservations. Guest details only flow **Aiosell → PMS** (OTA reservations via the inbound webhook `POST /api/public/aiosell/reservation`). So a *direct* booking never appears "as a booking" in Aiosell — only the available count drops. Don't tell users to look for a named booking in Aiosell.
- **Mapping is 1 Aiosell room ↔ 1 PMS room type.** In Step 2, **all** of an Aiosell room's rate plans (e.g. `executive-d-cp/-d-ep/-s-cp/-s-ep`) map to the **same** local room type. Cross-wiring (one Aiosell room → several types, or one type ← several Aiosell rooms) makes the availability push ambiguous/wrong. The `d/s`+`cp/ep` codes are occupancy + meal-plan pricing variants of one room, not separate rooms. `channel_room_mappings` GET joins `local_room_type_name`; the panel loads mappings **on mount** (don't gate the "Saved mappings" display behind the "Fetch from Aiosell" click).
- **Live auto-sync + owner automation.** Availability re-pushes automatically on hotel booking **create / modify / checkout / cancel** (finish-hook middleware `aiosellEventHook(evt)` → `handleAiosellBookingEvent` → debounced `scheduleAiosellResync`, off the request path — no added latency, no CF-502). Behaviour is **owner-configurable** per event (`aiosell_automation` table; GET/PUT `/hotel/aiosell/automation`): OTA action (`SYNC_AVAIL`/`+rates`/`NONE`) + manager alert, plus an **interval scheduled sweep** (5-min tick fires each property on its own `schedule_interval_minutes`). Defaults reproduce the prior hard-wired behaviour.
- **Every PMS↔Aiosell call must log.** Call `logAiosellSync(tenantDb, {direction, operation, trigger, summary, status, detail?, actor?})` (writes `aiosell_sync_log`, shown in the Sync Log tab). A new Aiosell call with no log line is invisible to the owner — always add one.
- **OTA no-show** = `POST /hotel/bookings/:bookingId/mark-no-show`: resolves the OTA booking id + channel off the booking's `channel_ref`/`booking_source` (`AIOSELL:<id>` / `AIOSELL:<channel>`), calls `aiosellMarkNoShow` for **booking.com / gommt only**, flags the PMS booking `NO_SHOW` (matches the nightly no-show sweep: void folio + notify). Surfaced as a per-row "Mark no-show" button on the Step 6 channel-bookings report.
- **Credentials are per-tenant** (hotelCode + partner id/user + AES-encrypted password on `channel_credentials`), with platform `AIOSELL_*` env vars as an optional shared fallback. Sandbox: partnerId `sample-pms`, hotelCode `sandbox-pms`.
- **Hotel booking status:** `BOOKED` **is** the confirmed/active state (→ `CHECKED_IN` → `CHECKED_OUT`, or `CANCELLED`, or `NO_SHOW`). There is **no** separate `CONFIRMED` status for hotel `room_bookings` — that exists only in the Events module.

#### Inbound OTA reservation — JSON → `room_bookings` mapping (`aiosellIngestReservation`)
Aiosell POSTs `{ action:'book'|'modify'|'cancel', hotelCode, bookingId, channel, checkin, checkout, pah, specialRequests, guest:{firstName,lastName,phone,email}, rooms:[{ roomCode, rateplanCode, occupancy:{adults,children}, guestName, prices:[{sellRate}] }], amount:{amountAfterTax, commission, tds, tcs, tax} }`. **The payload does NOT carry your category name** — the category is resolved from the mapping: `rooms[0].roomCode` (+ `rateplanCode`) → `channel_room_mappings` → `local_room_type_id` (your room type/category), then a **floating** room of that type (`room_locked=0`, Unassigned until check-in) is attached. Unmapped `roomCode` → 400, no booking. Field landing: `guest.firstName+lastName`→`guest_name` (fallback `rooms[0].guestName` → `"<channel> Guest"`); `guest.phone/email`→`guest_phone/guest_email`; `occupancy.adults+children`→`num_guests`; `checkin/checkout`→`check_in_date/check_out_date`; `amount.amountAfterTax` (else Σ`prices[].sellRate`)→`total_amount` (÷nights→`room_rate`); `amount.commission`→`commission_amount` (+derived `commission_pct`, `net_amount`); `pah`→`channel_prepaid` (`false`=prepaid/OTA-collected→1, `true`=collect-at-hotel→0); `amount.tds/tcs/tax`→`channel_tds/channel_tcs/channel_tax`; `bookingId`→`channel_ref="AIOSELL:<bookingId>"` (idempotency + cancel/modify/no-show match); `channel`→`booking_source="AIOSELL:<channel>"`; mapped rate plan→`rate_plan_id`+`rate_plan_snapshot`. Always `status=BOOKED`, `booking_type=OVERNIGHT`, `room_locked=0`.
...
### Restaurant dine-in, invoicing & GST single-source (2026-08-28 cycle)
* **GST is now single-sourced from Settings → GST** (marker `gst-noneditable-from-settings` → `gst-single-source-declutter`). The printed GST rate ALWAYS equals `restaurants.gst_percentage`; staff can never edit GST on a bill (the "Apply GST" toggles were removed from every restaurant invoice modal). `computeInvoiceTotals` (server.ts ~13104) no longer prefers a `tax_config` GST row's *rate* — it takes the rate from Settings and reuses an existing GST row only for its CGST/SGST **split** structure (`isGstTaxRow` helper). Fixes the "field shows 5, invoice shows 6 — 2 GST % inconsistent" bug. `tax_config` now carries **non-GST** lines only; the `TaxCurrencyPanel` GST line is hidden (and stripped on save) and that whole panel is collapsed as "Advanced — Currency & Non-India Tax" to declutter the settings page. **DECISION: the standalone-POS path no longer auto-bumps 5%→18% for "specified premises" (any room >₹7,500)** — that silently broke field==print; a specified-premises hotel now *declares* 18% in Settings, and the charge-to-room folio path keeps its own hotel-F&B rule. `TC-GST-FIELD-EQUALS-PRINT`.
* **India GST compliance gate** (marker `gst-compliance-gstin-gate`) — no GSTIN ⇒ GST cannot apply (dropped from invoices + forced off on save); setting/changing the GSTIN resets the rate to 5%; enabling GST with no rate → **400 `GST_RATE_REQUIRED`** (can't save). Settings `PATCH /api/restaurant/:id` (~43638) computes `effIsGstEnabled`/`effGstPct`. `TC-GST-COMPLIANCE-RATE/-NO-GSTIN/-RESET5` (destructive → snapshot + two-step restore).
* **Command Center table-bill → two-column touch layout** (marker `table-bill-two-column-layout`) — `PostpaidInvoiceModal` (App.tsx ~57300) restructured to match the Manual-Invoice screen the client prefers: wide `max-w-5xl` card, LEFT = order rounds, RIGHT rail = bill adjustments + grand total, payment/charge-to-room/print/close kept in the full-width footer. Pure JSX/layout — totals (server `preview-totals`) and all handlers unchanged.
* **CRITICAL table-clear fix** (marker `table-clear-fresh-session`) — clearing a table via Command Center left the session `open`, so the NEXT guest's order resumed the old session and inherited the previous bill's rounds. `PATCH /tables/:tableId/status`→AVAILABLE now closes any `open`/`bill_requested` session (`invoice_status = COALESCE(…,'DRAFT')`); order-create only resumes a still-open session. Staff-only. `TC-DINE-TABLE-CLEAR`, `TC-DINE-CLEAR-DRAFT`.
* **Cleared table is NOT auto-marked Paid** — freeing a table manually leaves the invoice **DRAFT/Active** unless actually settled; `isInvoicePaid()` (App.tsx) treats a SESSION as paid only when `status='closed' && payment_method && ≠'NONE'`.
* **Dine-in flow** — shared-floor waiter mode (`waiter_shared_floor`, all waiters see every table) + "Assign to all tables" bulk button (`POST /tables/assign-waiter-bulk`); natural table sort (`naturalCompare`/`byTableName`, N01<N02<N11); waiter-view search/filter above 8 tables; manager invoice add/remove items via `InvoiceItemsEditor` → added items become a separate **Manager Adjustment** round (`is_adjustment=1`, `POST /sessions/:token/adjustment`); manager invoice edit rejects an empty bill. `TC-DINE-*`, `TC-INVOICE-EMPTY-REJECT`.
* **RBAC Phase 0** (marker `issue-xlsx-rbac-gst-fixes`) — extracted `DEFAULT_ROLE_PERMS` (server.ts ~5288) and widened the operational seeds (WAITER/CHEF/CASHIER get `ORDERS:2`, HOUSEKEEPING gets `HOUSEKEEPING:2`, FEEDBACK added) so a future idempotent backfill can't lock the POS out; trimmed finance tabs from `RBAC_NEWLY_ADDED`; `ALWAYS_VISIBLE_TABS` narrowed to `{ROSTER,TIMESHEET,PROCUREMENT}`; `/loyalty/customers` gated `requireTabAccess('LOYALTY')`. Plan for Phases 1-2 (backfill + read-gating) in `docs/RBAC_HARDENING_PLAN.md`. See [[reference_rbac_visibility_leaks]].
...
### Thermal KOT auto-print (markers `thermal-kot-autoprint*`, `thermal-kot-config-ui`)
* **Print-routing layer** — browser `window.print()` can't silently target a specific printer, so KOTs print via an **on-prem agent**. New tenant tables `kitchen_printers` (name/station/host/port/copies/is_default) + `print_jobs` (queue) + per-tenant `restaurants.print_agent_token`. `enqueuePrintJobsForOrder()` fires on every public `POST /orders`: it groups items by menu **category** and inserts one job per matching-station printer (a printer with station `ALL` prints the whole order; else it matches the category). Owner CRUD `/kitchen-printers`, token `/print-agent-token[/rotate]`, and agent-only `/print-jobs/pending` + `/print-jobs/:jobId/ack` (auth via `X-Print-Agent-Token`, NOT the JWT). **LANDMINE: `requireOwnerOrAdmin` is an in-handler `boolean` guard, not Express middleware — the owner endpoints must call `if (!requireOwnerOrAdmin(req,res)) return;` inside the handler, never pass it as a middleware arg (doing so hangs the request).**
...
### Staff Access grid-prefill privilege escalation FIXED (marker `staffaccess-prefill-no-escalation`)
Root cause of "PCC/Security has no Events access but Quotations / Cleaning Checklist / Event Reports are visible": `fetchStaffAccess` (App.tsx ~14400) prefilled `EVENTS_CHECKLISTS`, `PROCUREMENT`, `EXPENSE_JOURNAL`, `CHECKLISTS` (+ Hotel Inventory / Spa tabs) to **Full(3)** into every role's in-memory matrix, and `saveStaffAccess` POSTs the **whole** object — so **opening + saving the Staff Access matrix silently granted those tabs** to roles set to N/A. One stray `EVENTS_CHECKLISTS` flips `hasEventsGrant` true → the Events group + Cleaning Checklist appear **and** the role passes the `eventsStaff` module gate (Events API read). The frontend prefill list had **drifted** from server `RBAC_NEWLY_ADDED` (which had dropped all four). Fix: (1) realign the prefill list to `RBAC_NEWLY_ADDED`; (2) **never prefill an AUTHORITATIVE role** (`CUSTOM_*` id, or matrix carries `__complete__`). Verified on RESTO-1003: old logic would leak **597 sensitive grants across 212 roles on the next Save → 0**. `TC-XLSX-RBAC-GRID-PREFILL`. Existing pollution doesn't self-heal — owner re-saves with the tabs set to N/A (a preview-then-bulk-clean was run on RESTO-1003; other tenants need their own owner/super-admin access). See [[reference_rbac_visibility_leaks]].

### Cashier → therapist "My Schedule" misroute FIXED (marker `cashier-therapist-misroute-fix`)
Same root cause: the prefill had injected `SPA_CALENDAR`/`SPA_APPOINTMENTS` into a custom Cashier role, and the Home router (App.tsx ~16057) sent any `CUSTOM_*` role holding a spa tab to `<TherapistDashboard>` (which demands a linked therapist profile → "set your Staff ID in Therapist settings"). Fix: the spa→Therapist home route now also requires the role to hold **no clearly-non-spa operational tab** (`CASH_DRAWER`/`INVOICES`/`ORDERS`/finance/…) — a genuine spa-only therapist still lands there; a cashier with a stray spa tab falls through to the launchpad. Fixes already-polluted roles immediately. `TC-XLSX-THERAPIST-ROUTE-GUARD`.

### Command Centre Map flicker FIXED (markers `command-centre-flicker-fix` → `floorplan-remount-flicker-fix`)
Two causes: (1) `alert-pulse` (a 1.3s infinite glow for TRANSIENT alerts) was applied to every SLOW-turn tile — several slow tables pulsing at once flickered; scoped to bill-requests only + added a `prefers-reduced-motion` guard. (2) **The real one** — `FloorPlanMap` defined its sub-views (`SectionTabs`, `ViewMode` = the whole tile grid, `ArrangeMode`, `TileBody`) as components **inside** the component and rendered them as `<ViewMode/>`; a nested component definition gets a new identity every render → React **unmounts+remounts the entire grid on every re-render** (every 30s live-refresh + every parent re-render). Fix: invoke them as functions `{ViewMode()}` etc. so the JSX inlines and the `key={t.id}` tiles reconcile. **RULE: in a big component, never render an inline-defined helper as `<Helper/>` — call it `{Helper()}`.** See [[reference_floor_plan_build]].

### Test scripts are NOT tracked in git (owner preference, 2026-09-02)
`test-scripts/` and `run-access-audit.bat` are gitignored and untracked — kept **local only** (the files stay on disk and still run; they're just never committed). **Do NOT re-commit test scripts.** The deploy (`vite build` + `tsx server.ts`) doesn't need them.

## Recent Feature Additions (2026-09 cycle — full UAT sweep: GST serials, order integrity, ledger precision, Menu white screen, checklist audit)

> Method used for all four UATs (7–8 Sep 2026, tenant **RESTO-1003 "Manhotra Consulting"**, run against **production**): a scripted scenario suite in `scratchpad/uat_*.mjs` drives the real API as the owner + purpose-made staff logins, records PASS/FAIL per scenario tagged **happy / unhappy**, creates only **throwaway masters** (rooms, halls, templates, menu items, staff), and **reverses every financial effect** (bills cancelled → GL contra-journal; folios reversed by credit note) in a `finally` block. Findings are then fixed, deployed, and **re-tested live**. Reports are published as artifacts ("Manhotra Consulting UAT", "Manhotra Restaurant UAT", "Manhotra Checklist UAT"). **Definition of done for any fix in this cycle: `node test-scripts/run_technical_tests.mjs` with the only FAIL being the pre-existing `TC-EXPENSE-SHARED`** (current: 278 pass / 18 skip / 1 fail).

### Hotel + Events UAT — 8 findings, all fixed (markers `gst-serial-unify-f1f2f3`, `uat-f4-f8-guards-catering-gst`; commits `1d70473`, `803a229`)
* **ONE hotel document-serial allocator** — `allocateFolioSerial` mints `INV-<FY>-NNNNN` / `CN-<FY>-NNNNN` (FY via `getYearIST`). `ensureFolioInvoiceNumber` mints **only** for `settled|superseded|closed`; an open folio renders a non-persisted **`PROFORMA-<last6>`** and the PDF title flips to "PROFORMA INVOICE" (`isProforma` → `invoiceService.ts` / `invoiceServiceBoutique.ts`, label in `invoiceServiceShared.ts`). **Rendering an invoice must never mint a number.** The GST register now receives the minted number at settlement (`writeGstRegisterFromFolio(..., {invoiceNumber})`) instead of writing NULL and back-filling later.
* Room delete is **409 `ROOM_HAS_ACTIVE_BOOKINGS`** while BOOKED/CHECKED_IN bookings reference it; event `complete` requires IN_PROGRESS (or CONFIRMED whose date has arrived) and is idempotent once COMPLETED; catering lines bill at the **package's** GST%, not the event default; a credit note is refused unless the parent folio is `settled`.

### Restaurant + Restaurant↔Hotel UAT — 9 findings + a ledger defect, all fixed (markers `rest-order-integrity-f-r1-r2-r9`, `rest-guards-f-r3-r8-ledger-numeric-f-a1` → `ledger-numeric-f-a1-migration-b`; commits `af5b6f6`, `ee4308d`, `2e34add`)
* **GL == printed bill.** `_postOrderGl` derives every order journal from the order's **items** via `globalThis.__computeInvoiceTotals` (the same engine that prints the bill), so the storage convention can't skew the ledger. **CONVENTION (keep!): `orders.total_amount` on a round is the PRE-tax subtotal with `gst_amount` alongside**; manual/edited invoices store inclusive totals; itemless legacy rows keep the inclusive reading.
* **`POST /orders` validates and recomputes.** ≥1 named item, integer qty ≥1, price ≥0 (400 `NO_ITEMS`/`ITEM_QTY_INVALID`/…); totals are **always** recomputed server-side (client values ignored). A **guest** (no valid staff JWT for the tenant — `_optionalStaffUser`) may order only menu items at menu prices (`ITEM_NOT_ON_MENU`/`ITEM_PRICE_MISMATCH`/…); staff keep custom items. Guests can't append to a `bill_requested` table (409), nobody can append to a closed/settled/cancelled session, and a stale token no longer opens a second session.
* **One sequential serial per session** — a number is drawn only when the session has none (was burned on every round).
* Charge-to-room manual invoices **bypass** `computeInvoiceTotals` (raw items, no restaurant GST) so the invoice equals the folio line; `_markRoomChargedOrdersPaid(folioId)` marks room-charged orders PAID at folio settlement; bill close guards (404 unknown / 409 already-settled); menu name+price validation; empty manual invoice → 400 before a serial is drawn.
* **F-A1 ledger precision:** `gl_entries.dr_amount/cr_amount` were `REAL` → float4 `SUM()` drifted paise in TB/BS/cash-flow/aging. Migrated **once per tenant** to `NUMERIC(14,2)` in `_initTenantDb` and `pgTypes.setTypeParser(1700, parseFloat)` in `db.ts` so numerics still arrive as JS numbers. **LANDMINE: the tenant `PostgresDb.exec()` splits SQL on `;` — a `DO $$…$$` block is fragmented and fails silently under `.catch`; use `db.get`/`db.run` for anything with internal semicolons** (that's why this needed a second commit).

### CRITICAL — "Restaurant → Menu white screen" was ONE bad row (marker `menu-null-dietary-white-screen`, commit `3abe21a`)
A menu row left behind by a **debug script** (`POST /menu` with no `dietary_type`) stored NULL; the Menu tab renders `item.dietary_type.replace(...)` → the whole React tree threw → blank app. Deleting the row fixed it instantly (no deploy). Hardening shipped: dietary badge + public-menu description search are null-safe; `POST /menu` and `PATCH /api/menu/:id` default a blank dietary type to `VEG` and description to `''`; **new `src/components/TabErrorBoundary.tsx` wraps the whole tab content column** (`resetKey={activeTab}`) so a render error shows a contained card instead of a white screen. `TC-MENU-NULL-DIET`.
* **RULE — white screen ⇒ suspect DATA first.** Call the tab's list endpoint and look for `null` in any field the JSX calls a string method on, *before* re-reading FE code or git history. (The other known cause is the stale PWA bundle — see the service-worker section.)
* **RULE — a diag/scratchpad script MUST delete every row it creates.** This one survived six days and took down a page for the owner.
* **LANDMINE — there is no `@types/react` in this project** (React 19 + `allowJs`). A class component's inherited `props`/`state`/`setState` are untyped: `declare` them on the class or `npx tsc --noEmit` fails. `vite build` passes anyway (esbuild doesn't type-check), so **always run both**.

### Checklist / Housekeeping UAT — 60 scenarios, 56 pass, findings OPEN (nothing fixed yet)
Config CRUD/validation, manual start + dedupe, mandatory-task gating, override, My Checklist buckets/remarks/assign, board filters, the **check-in gate** (409 `checklist_incomplete` → tick → 200), **check-out hold-and-release** (room CLEANING until the blocking departure checklist completes; staff 409 vs owner force-vacant which closes the job as an override), **two-stage cleaning**, the owner's validate-on-check-in/check-out toggles, room/venue status triggers and the post-event trigger **all behaved**. Open findings:
* **F-C1 (High)** — `runTenantScheduledChecklists` does `String(check_in_date).slice(0,10)` on a **pg DATE (a JS Date)** → `"Tue Sep 08"` → `Date.parse` → year 2001 → *nights ≈ 9131* (dedupe keys `…:N9131`). MID_STAY/CLEANING cadence is wrong and the departure-day skip never fires. **F-C2 (Medium)** — same root: `_chkYmd(Date)` fails its ISO regex so the CHECK_OUT job gets `due_date = NULL` and overdue reminders can't see it. **Fix both with the existing `normaliseDateIso()` (server.ts ~1987) before the night arithmetic.**
* **F-C3 (Medium) — FIXED** (commit `fedaf02`, marker `checklist-override-authority-f-c3`). Job **override** accepted `_roleHasTab('HOUSEKEEPING', 2)` while force-vacant was `HK_MANAGER_ROLES` only → an Edit housekeeper refused from freeing a room could override the job that held it, and no CUSTOM supervisor role could force a room at all. **RULE: skipping a release-blocking checklist is ONE privilege — always go through `_canOverrideChecklist(req)`** (built-in manager/owner OR `HOUSEKEEPING` at Full(3), the standard sensitive-action rule); Edit still ticks + completes normally. Found in the same pass: `POST /events/bookings/:bid/confirm` let **any** caller skip the open-venue-cleaning gate with `override_cleaning: true` (the role was only read to word the 409 message) — now gated by the same helper + an audit row. FE hides the Override button unless `canDeleteTab('HOUSEKEEPING')`. Smoke: TC-CHK-OVERRIDE-AUTH, TC-CHK-FORCE-VACANT-AUTH, TC-CHK-EVENT-CLEANING-BYPASS. **TEST LANDMINE: a custom role created with `scope:'HOTEL'` has its `EVENTS_*` grants STRIPPED at read time** (403 at the events module gate) — use `scope:'ALL'` for a role that must touch both modules.
* **F-C4/5/6 (Low)** — ticking an unknown task id returns 200 (no rowcount check); `run-scheduled` counts deduped existing jobs as "raised"; `releaseFacility` runs on complete/override of **non-blocking** jobs too, so closing an inspection can flip a CLEANING room to VACANT.
* **F-C7/F-C8 (Confirm, owner)** — RESTO-1003 has **504 room templates, all inactive**, so in daily use nothing gates check-in and nothing tracks cleaning; and the board carried 472 open jobs (hall-daily jobs dated to 2099, 73 post-event, 93 orphaned smoke-suite arrival jobs). 93 orphans closed + a leftover August `E2E Hall Daily` template deactivated → 386; the smoke suite now closes the job `TC-CHK-CHECKIN-NONBLOCK` raises.
* **LANDMINE — never leave an unassigned `MID_STAY`/`DAILY`/`CLEANING` template active while calling `POST /checklists/run-scheduled`.** Template resolution falls back to "active template with NO assignment" as the tenant-wide default, so a test template fires against **every real in-house stay / room / venue** (this run attached 32 turndown jobs to 16 real guests; all closed by override with room statuses preserved). Always assign UAT templates to throwaway rooms.
* Venue status vocabulary mirrors rooms: `VACANT|OCCUPIED|CLEANING|MAINTENANCE|BLOCKED` (there is no `AVAILABLE`). `DELETE /checklists/templates/:id` is a **soft** delete (`is_active=0`).

### Events — two sessions a day, and venue conflicts by real span (marker `event-overnight-sessions-and-note`, commit `3f4cb84`)
A property may sell each hall as **two fixed sessions a day** (this client: Morning 03:00→15:00, Night 15:00→03:00, flat per session, never hourly — rate basis `HALF_DAY` with `half_day_am_rate`/`half_day_pm_rate`, both falling back to `half_day_rate`). `venueBookingConflict` (eventsService.ts) used to treat a booking carrying an `end_date` as occupying those days **in full**, so a night session running past midnight claimed the whole next day and the next morning's session was refused as unavailable. It now reduces each booking to **one absolute span** (`dayNum*1440 + hhmm`, `end += 1440` when end ≤ start so a window past midnight ends the next day) and conflicts only on real overlap, widened by `turnaround_min` on each side; the SQL prefilter is widened ±1 day so an overnight candidate is never missed. Edge-exclusive, so a session may start exactly when the previous one ends. Also added the **Special note** box to the staff event booking form (the field, the save path and the public enquiry form already had it). `TC-EVT-NIGHT-THEN-MORNING`, `TC-EVT-BACK-TO-BACK`, `TC-EVT-REAL-CLASH`.
* **LANDMINE: `PUT /events/venue-settings` rewrites every field it owns** (`b.hd_am_start || '08:00'`, …) — echo the whole object or the session windows silently reset to the defaults. Same rule as `PATCH /hotel/settings`.
* **A turnaround gap blocks back-to-back sessions** — with `default_turnaround_min` 120 the same-day morning→night pair clashes by design. A property running sessions back to back needs 0 (set on RESTO-1003).
* **The 11-to-11 rule is a CHARGE, not a block** — on BOTH sides. `hotel_late_checkout_time` auto-adds one extra night past the cutoff on the departure date; `hotel_check_in_time` (+ `hotel_early_checkin_charge`, default OFF) does the mirror for arrivals via `computeEarlyCheckinFee` / `addEarlyCheckinFolioEntry`, waivable with `waive_early_checkin`, and is published on the booking confirmation. **Neither ever refuses a check-in or check-out** — a clean room at 08:00 should go to the guest; the only question is whether it is paid for. The two new fields save through COALESCE (a partial PATCH can't blank them); the older stay/refund fields still must be echoed.
* **LANDMINE — validate clock settings with `HHMM_RE`** (`/^([01]\d|2[0-3]):[0-5]\d$/`). The old `/^\d{2}:\d{2}$/` checked shape only and accepted **`25:99`**, which stores as minute 1599 — later than any wall clock, so an early-arrival charge fires on EVERY check-in and a late-checkout cutoff never fires. Caught by its own new smoke case on the first deploy.

### Events — customer GST details on the invoice (marker `event-customer-gst-details`, commit `c84f519`)
A customer claiming **input tax credit** needs its name, ADDRESS and GSTIN on the tax invoice (Rule 46). `customer_gstin` existed and showed on the on-screen folio, but there was no address column and neither reached the PRINTED PDF. Added `event_bookings.customer_address` + both fields rendered in the PDF "Prepared For" block **only when present** (a walk-in invoice is unchanged). **The details live on their own route — `PUT /events/bookings/:bid/gst-details` — deliberately NOT the booking editor:** they carry no money, so they are settable on a **COMPLETED** booking (whose editor still 409s, protecting the amounts), which is exactly when a company asks for a GST bill — fill in and reprint. Validates the 15-char GSTIN, requires an address alongside it, `''` clears, 409 on CANCELLED, audited `GST_DETAILS_UPDATED`.
* **LANDMINE — never assert on PDF bytes.** PDFKit **deflates** content streams and writes text as **hex inside kerned TJ arrays**, so `pdfBuffer.includes('27AAPFU…')` never matches even when the text is on the page. Inflate each stream, keep the ones containing `Tj`/`TJ`, then decode `<hex>` **two digits per character** (four = UTF-16 garbage). See the `pdfText` helper in `run_technical_tests.mjs`. Also: the invoice header always prints the **property's own** GSTIN, so a "customer has no GST details" assertion must look for the customer's specific number, not the label.

## Recent Feature Additions (2026-09 cycle — bank reconciliation: review, 6-stage remediation, statement, import)

> A 14-finding review of the bank reconciliation feature, then a **6-stage remediation plan written before any code**, each stage shipped as its own commit + marker + live smoke run. Artifacts: review, plan and user manual (see `project_bankrec_remediation` in memory). All 14 findings are closed. Markers `bankrec-stage0-…` → `bankrec-stage6b-import-charges`.

### The core defect — ticking "Cleared" did nothing (F-1, F-2)
`difference` was `book_balance − statement_closing_balance` and the UI repeated the same subtraction; the cleared set only **decorated rows**. `reconciled` meant book == statement, which at a real month end is almost never true. The real identity now drives the screen:
* **`adjusted_book_balance = book − uncleared_deposits + uncleared_withdrawals`** — an uncleared **DEBIT** is a deposit in transit (our books have it, the bank does not), an uncleared **CREDIT** is an unpresented cheque. Summed over **every entry ≤ the statement date, not just the window** — a cheque issued 28 Aug and cleared 3 Sep is absent from September's list and must still count.
* The printable statement states the inverse and agrees by construction: **statement balance + deposits in transit − cheques not presented = the books**.

### LANDMINE — `_acctOwnerOnly` is the READ gate and must NEVER guard a write
`_acctOwnerOnly` (26 routes) admits **any role with ≥ View on ANY finance tab**. `_acctCanWrite` (17 routes) requires **`ACCOUNTING` ≥ Edit(2)** plus the built-in OWNER/SUPER_ADMIN/CTO/**MANAGER** roles by name. They are a deliberate **read/write split**, not a loose and a tight version of one gate, and a comment under the helper says so. In stage 4 I read a finding as "the read is tighter than the write" and moved both bank-rec POSTs onto `_acctOwnerOnly` — that **LOOSENED** the write and re-opened the known "View-only can still write" bug class; reverted within the hour (`bankrec-stage4b-revert-write-gate`). The live test even showed a MANAGER *allowed* to write and I read it as confirmation. **Also: never edit a shared helper to fix one route — change which helper the ROUTE calls.** Guarded now by `TC-ACC-BANKREC-GATES`, which provisions a custom role holding Ledger & Books at **View only** — the one shape that tells the two helpers apart — and asserts read 200 / save 403 / sign-off 403.

### LANDMINE — a boot-time backfill that re-imports from a table allowed to diverge WILL destroy data
`db.ts` backfilled `bank_rec_cleared` → `bank_cleared` on **every** boot, commented "idempotent, so it is safe on each boot". It cannot duplicate rows; it can do worse. Unticking removes the durable row, so every restart **resurrected ticks the user had deliberately removed** and silently changed a reconciliation's arithmetic — the same defect as the stage-3c bug, arriving through the migration instead of the read path. 12 rows were already queued to come back on the live tenant. Fixed in `bankrec-stage5b-no-resurrect`: untick clears the legacy shadow for the whole **account** (not one reconciliation); a one-time cleanup drops orphaned legacy rows **before** the backfill; and the backfill is **one-shot** behind a `bank_cleared_backfill` marker table. The save also writes the **authoritative** row first and no longer swallows its error — it used to write the legacy row unguarded and the durable one inside a `.catch`, so a failed insert lost the tick silently.
* **RULE — the display and the arithmetic must use the SAME cleared set.** Violating it (display unioned legacy + durable, arithmetic read durable alone) let a line render as ticked while still counted outstanding. `TC-ACC-BANKREC-COHERENT` rebuilds the totals from the rendered flags and requires them to equal the server's.

### LANDMINE — paging a list you can TICK is a protocol change, not a display change
The save unticked anything **in the window** absent from the payload, because absence meant removal. Once the list is paged, an ordinary save from page 2 would have **silently wiped every tick on page 1**. The client now sends `visible_entry_ids` and only those may be unticked; without the field the old window-wide rule stands, so a stale cached bundle behaves as before. Guarded by `TC-ACC-BANKREC-PAGESAFE`. Paging also broke the **arithmetic**: the browser summed the unticked lines *on screen* plus a server figure for everything outside the window — right only while the screen held the whole window. Inverted: the server's whole-history totals are authoritative and the browser moves them **only by the ticks the user has changed**.

### Statement import — it PROPOSES, a person CONFIRMS (F-10)
`POST .../accounting/bank-reconciliation/match` parses a bank CSV and pairs rows with **uncleared** movements: exact on amount, within N days on date (default 3, max 15), each entry claimable once, closest date wins, a tie returned flagged `ambiguous` rather than silently picked. **It writes nothing** — no tick, no journal; the ordinary save applies it. `TC-ACC-BANKREC-IMPORT-SAFE` asserts the cleared-line count and GL-exception count are unmoved by a match.
* **SIGN CONVENTION — a statement DEBIT is money leaving the bank, which is a CREDIT in our books.** Backwards, nothing matches.
* Parser handles what Indian statements actually look like: dd/mm/yyyy, dd-mm-yy and ISO; separate Debit/Credit columns or one signed Amount; thousands separators and `Rs.` prefixes; quoted narrations containing commas; and **preamble lines above the header** (it scans the first 12 lines for the row that looks like a header — without that, every real export failed with "no date column").
* A row splitting into **more columns than the header** is refused with a reason: an unquoted `Rs. 2,100.00` shears on its comma and yields amounts wrong by orders of magnitude, and reporting that as "nothing matched" sends someone hunting the wrong problem.
* **A test that demands specific matched ids cannot pass on real data** — this tenant holds eight identical ₹111.00 movements on one date, so any is an equally correct answer. Assert the invariants instead: every row paired, ids distinct, amounts exact, gaps within tolerance, duplicates flagged.

### Reconciliation statement PDF (F-3) — and two PDFKit landmines
New `bankRecStatementPdf.ts` + `GET .../bank-reconciliation/statement` (the model, JSON) and `.../statement.pdf`. **Deliberately separate routes** so a rendering failure cannot take loading or saving down with it. Serving the **model** as JSON is what makes it testable — a PDF's text is deflated and hex-encoded and cannot be searched from a test — so the figures are asserted against the JSON (including a cross-check that the document's itemised totals equal the screen's uncleared sums) and the PDF test only proves a real PDF comes back.
* **PDFKit's built-in Helvetica has no ₹ (U+20B9) and THROWS on it** — money is `Rs.`, the same fix the hotel invoice and event quotation templates use.
* **`ellipsis: true` does NOT truncate — it wraps**, and the second line prints on top of the row beneath. Clip by measuring with `doc.widthOfString`.
* **RULE — render the PDF locally (`npx tsx`), rasterise it (pymupdf) and LOOK at it before deploying.** Four shapes (normal / no statement balance / 412 items over 7 pages / empty) caught the overprint that no type check or smoke test would have.

### Bank charges post through the ORDINARY expense route (F-12)
A charge on the statement that is not in the books blocks the reconciliation forever. It posts via the existing `expense-payments` route (interest credited via the existing manual journal) so it lands in the **Expense Journal**, visible and reversible by machinery that already exists — not in a private corner only this screen knows about. New COA account **`5520 Bank Charges`** (the nearest before were `5800 Petty Cash Expenses`, the catch-all default, and `5510 Card & UPI Charges` — both mislabel the P&L). `expense-payments` gained an optional **`bank_account_code`**: it always credited `1010`, so a property with two bank accounts had every expense booked against the first. It is **validated against `bank_accounts`** so it cannot credit an arbitrary GL account.

### Also shipped across the stages
Durable per-account clearing (`bank_cleared`, so a tick outlives the period it was made in) · reconciliations keyed on **account + statement_date** (not the literal `from..to` text, which orphaned the saved balance when a date was nudged) with UPSERT, history list and DRAFT/FINAL sign-off (**409 `RECONCILIATION_FINAL`**) · `opening_balance` + `window_movement` with the invariant **opening + net = book_balance** · the account picker driven from `bank_accounts` instead of two hardcoded codes, one of which (`1020`) nothing ever posts to · search / uncleared filter / sort / running balance (**offered only on the unfiltered date-ordered list**, the one arrangement where it means anything) · `cleared_on`/`cleared_by` surfaced per line.

**The stage-1 dual write is still in place.** The plan gates its removal on a release having proved the new table. `legacy_ticks_unmigrated` on the reconciliation read + `TC-ACC-BANKREC-DUALWRITE` assert it is nil — **that assertion is what caught the resurrection bug above**. While it stays nil, `bank_rec_cleared`, the dual write and the field come out in one commit.
...
* **Vite production build** (`npx vite build`) must succeed cleanly before pushing. **Run `npx tsc --noEmit` too** — the Vite build uses esbuild and does NOT type-check, so a type error ships silently (this project also has no `@types/react`; see the Menu white-screen section).
* **Smoke suite after every change** — `node test-scripts/run_technical_tests.mjs`; the only acceptable FAIL is the pre-existing `TC-EXPENSE-SHARED` (baseline as of 2026-09-11: **347 total / 328 pass / 18 skip / 1 fail**). Add a test case for every fix. `test-scripts/` is **gitignored** — the suite is local only. It uses dedicated rooms **`ZZ Smoke Test 1..4`** (floor 99) on RESTO-1003 via `smokeRooms()`; it used to grab `rooms[0]`, which demo data occupies, so hotel/group cases silently skipped and coverage thinned without anyone noticing.
* **Re-running the suite back to back trips the API rate limiter** (HTTP 429 `Too many requests`) and produces a screen of failures that are nothing to do with the code. Wait a few minutes between full runs before believing a sudden cluster of 429s.
* **Scripts must clean up after themselves** — any script (smoke, UAT, scratchpad diag) that creates a row on a live tenant must remove or close it in a `finally` block: menu items, bookings, rooms, halls, staff, roles, **and the checklist jobs a booking raises** (cancelling the booking does not close them). A stray debug row took down the Menu page for six days.
