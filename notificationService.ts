  ...
  switch (eventName) {
    ...
    case 'HOTEL_INVOICE_SENT':
      return {
        ...
        html:
          `<h2 style="color:${brandHex()}">${data.isCreditNote ? 'Credit note' : 'Your invoice'}</h2>` +
    ...
    case 'PAYMENT_LINK_SENT':
      return {
        ...
        html:
          `<h2 style="color:${brandHex()}">Payment request</h2>` +
          `<p>Dear ${data.customerName || 'Guest'}, please pay <strong>${data.amount || ''}</strong> to <strong>${r}</strong>` +
          `${data.purpose ? ` for ${data.purpose}` : ''}.</p>` +
          `<p><a href="${data.payUrl || '#'}" style="display:inline-block;padding:12px 24px;background:${brandHex()};color:#fff;border-radius:10px;text-decoration:none;font-weight:bold">Pay ${data.amount || ''}</a></p>` +
    ...
    case 'HOUSEKEEPING_REQUESTED':
      return {
        ...
        text:
          ...
          `Ref: ${data.requestId}`,
        html:
          `<h2 style="color:${brandHex()}">🧹 New guest request</h2>` +
    ...
    case 'BOOKING_CREATED': {
      ...
      const totalHtml = amount > 0
        ? `<p>Total: <strong style="color:${brandHex()}">${currencySymbol}${amount.toLocaleString('en-IN')}</strong></p>`
        : '';
      return {
        ...
        html:
          ...
          `<h2 style="color:${brandHex()};margin-top:0">📅 Booking confirmed</h2>` +
          ...
          `</div>` +
          (hasUpi ? (
            `<div style="border-top:2px dashed ${brandHex()};padding-top:16px;margin-top:16px">` +
            `<h3 style="color:${brandHex()};margin-top:0">💰 Pay direct via UPI</h3>` +
            ...
            `<a href="${upiLink}" style="background:${brandHex()};color:#fff;padding:14px 28px;border-radius:8px;text-decoration:none;font-weight:bold;display:inline-block">Pay ${currencySymbol}${amount.toLocaleString('en-IN')} via UPI</a>` +
            `</p>` +
    ...
    case 'GUEST_PRE_ARRIVAL':
      return {
        ...
        html:
          `<div style="font-family:sans-serif;max-width:600px;margin:auto;padding:24px;border:1px solid #e5e7eb;border-radius:8px">` +
          `<h2 style="color:${brandHex()};margin-top:0">🏨 See you soon at ${r}!</h2>` +
          ...
          `<a href="${data.checkinUrl || '#'}" style="background:${brandHex()};color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:bold">Complete Online Check-In</a>` +
          `</p>` +
          `<div style="background:#faf7f2;padding:16px;border-radius:8px;margin:16px 0">` +
          `<p style="margin:0 0 4px 0"><strong>Your stay</strong></p>` +
    ...
    case 'NEW_PLATFORM_ORDER': {
      ...
      return {
        ...
        text:
          ...
          unmappedLine,
        html:
          `<h2 style="color:${brandHex()}">🍱 New ${channel} Order</h2>` +
          `<table cellpadding="6" style="border-collapse:collapse;font-size:14px;">` +
    ...
    case 'WEBHOOK_SIGNATURE_FAILURE':
      return {
        ...
        html:
          ...
          `<p><a href="#" style="color:${brandHex()}">Open Settings → Integrations → ${data.channel} to rotate credentials.</a></p>`,
      };
    ...
    case 'LOYALTY_TIER_UPGRADED': {
      ...
      return {
        ...
        html:
          `<div style="font-family:sans-serif;max-width:600px;margin:auto;padding:24px;border:1px solid #e5e7eb;border-radius:8px">` +
          `<h2 style="color:${brandHex()};margin-top:0">🎉 Welcome to ${tier}!</h2>` +
          `<p>Hi <strong>${name}</strong>,</p>` +
          `<p>You've just been upgraded to <strong>${tier}</strong> at <strong>${r}</strong>.</p>` +
          (pct > 0 ? `<div style="background:#fef0e4;border:1px solid #fcd34d;border-radius:8px;padding:14px;margin:16px 0;text-align:center"><strong style="color:${brandHex()};font-size:22px">${pct}% OFF</strong><br><span style="color:#3d3128">automatically applied to every order, forever.</span></div>` : ``) +
    ...
    case 'LOYALTY_REWARD_AVAILABLE': {
      ...
      return {
        ...
        html:
          `<div style="font-family:sans-serif;max-width:600px;margin:auto;padding:24px;border:1px solid #e5e7eb;border-radius:8px">` +
          `<h2 style="color:${brandHex()};margin-top:0">${tier} reward ready</h2>` +
          `<p>Hi <strong>${name}</strong>,</p>` +
          (pct > 0
    ...
    case 'SHIFT_ASSIGNED': {
      ...
      return {
        ...
        html:
          `<div style="font-family:sans-serif;max-width:600px;margin:auto;padding:24px;border:1px solid #e5e7eb;border-radius:8px">` +
          `<h2 style="color:${brandHex()};margin-top:0">New shift assigned</h2>` +
          `<p>Hi <strong>${name}</strong>,</p>` +
    ...
    case 'SHIFT_UPDATED': {
      ...
      return {
        ...
        html:
          `<div style="font-family:sans-serif;max-width:600px;margin:auto;padding:24px;border:1px solid #e5e7eb;border-radius:8px">` +
          `<h2 style="color:${brandHex()};margin-top:0">Shift updated</h2>` +
          `<p>Hi <strong>${name}</strong>,</p>` +
    ...
    case 'CHECKLIST_ASSIGNED': {
      ...
      return {
        ...
        html:
          `<div style="font-family:sans-serif;max-width:600px;margin:auto;padding:24px;border:1px solid #e5e7eb;border-radius:8px">` +
          `<h2 style="color:${brandHex()};margin-top:0">New checklist assigned</h2>` +
    ...
    case 'SHIFT_REMINDER': {
      ...
      return {
        ...
        html:
          `<div style="font-family:sans-serif;max-width:600px;margin:auto;padding:24px;border:1px solid #e5e7eb;border-radius:8px">` +
          `<h2 style="color:${brandHex()};margin-top:0">Shift reminder</h2>` +
          `<p>Hi <strong>${name}</strong>,</p>` +
    ...
    case 'LOYALTY_BIRTHDAY_REWARD': {
      ...
      return {
        ...
        html:
          ...
          `<h2 style="color:${brandHex()};margin-top:0">🎂 Happy birthday, ${name}!</h2>` +
    ...
    case 'LOYALTY_NEAR_UPGRADE': {
      ...
      return {
        ...
        html:
          ...
          `<h2 style="color:${brandHex()};margin-top:0">You're almost ${nextTier}!</h2>` +
          `<p>Hi <strong>${name}</strong>,</p>` +
    ...
    case 'FEEDBACK_REQUEST': {
      ...
      return {
        ...
        html:
          `<div style="font-family:sans-serif;max-width:600px;margin:auto;padding:24px;border:1px solid #e5e7eb;border-radius:12px">` +
          `<h2 style="color:${brandHex()};margin-top:0">How was your visit?</h2>` +
          `<p>Hi <strong>${name}</strong>,</p>` +
          `<p>Thanks for visiting <strong>${r}</strong>! We'd love to hear how it went.</p>` +
          `<p style="text-align:center;margin:24px 0">` +
          `<a href="${link}" style="background:${brandHex()};color:#fff;padding:14px 28px;border-radius:12px;text-decoration:none;font-weight:700;display:inline-block">Rate your visit</a>` +
          `</p>` +
    ...
    case 'FEEDBACK_OWNER_REPLY': {
      ...
      return {
        ...
        html:
          `<div style="font-family:sans-serif;max-width:600px;margin:auto;padding:24px;border:1px solid #e5e7eb;border-radius:12px">` +
          `<h2 style="color:${brandHex()};margin-top:0">Thanks for your feedback</h2>` +
          `<p>Hi <strong>${name}</strong>,</p>` +
    ...
    case 'EVENT_UPCOMING_REMINDER':
      return {
        ...
        html:
          `<div style="font-family:sans-serif;max-width:600px;margin:auto;padding:24px;border:1px solid #e5e7eb;border-radius:8px">` +
          `<h2 style="color:${brandHex()};margin-top:0">📅 Your event is coming up!</h2>` +
