...
const rows = [];
const ok = (c, id, msg, note = '') => { c ? passN++ : failN++; rows.push({ id, name: msg, status: c ? 'PASS' : 'FAIL', note }); console.log(`${c ? '✅ PASS' : '❌ FAIL'}  ${id} — ${msg}${note ? '  | ' + note : ''}`); };
const skip = (id, msg, note = '') => { skipN++; rows.push({ id, name: msg, status: 'SKIP', note }); console.log(`⚠️  SKIP  ${id} — ${msg}${note ? '  | ' + note : ''}`); };
...
(async () => {
  ...
  try {
    ...
    // R6 FIX: the CHECK_OUT (checkout) checklist must NOT be raised at check-in — only
    // at actual check-out. A checkout checklist appearing right after check-in was the QA bug.
    const preCoJobs = (await jobsFor(roomId, 'CHECK_OUT')).filter(j => created.templates.includes(j.template_id));
    ok(preCoJobs.length === 0, 'E2E-NO-CO-AT-CHECKIN', 'Check-out checklist is NOT raised at check-in (only at actual check-out)', `checkout jobs after check-in=${preCoJobs.length}`);
    ...
    try {
      const badge = (s) => s === 'PASS' ? '✅ PASS' : s === 'FAIL' ? '❌ FAIL' : '⚠️ SKIP';
      const body = rows.map(r => `| ${r.id} | ${r.name} | ${badge(r.status)} | ${(r.note || '').replace(/\|/g, '\\|')} |`).join('\n');
      const failed = rows.filter(r => r.status === 'FAIL');
      const md = `# Atithi-Setu — Checklist Trigger E2E Report\n\n**Base URL:** ${BASE}\n**Restaurant ID:** ${RID}\n\n## Summary\n| Metric | Count |\n|---|---|\n| ✅ Passed | ${passN} |\n| ❌ Failed | ${failN} |\n| ⚠️ Skipped | ${skipN} |\n| **Total** | **${rows.length}** |\n\n## Results\n| ID | Test | Status | Notes |\n|---|---|---|---|\n${body}\n${failed.length ? `\n## Failed\n${failed.map(f => `- **${f.id}**: ${f.name} — ${f.note || ''}`).join('\n')}\n` : ''}\n---\n_Generated by test-scripts/e2e_checklists.mjs_\n`;
      writeFileSync(join(__dirname, 'CHECKLIST_E2E_REPORT.md'), md);
      console.log('Report written → test-scripts/CHECKLIST_E2E_REPORT.md');
